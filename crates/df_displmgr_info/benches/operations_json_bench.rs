...
use df_displmgr_info::{
    edid_types::{
        AudioCapabilities, AudioMuteState, ChromaticityCoordinates, DeepDdcStats, EdidData,
        HdrMetadata, InputSource, MonitorCapabilities, MonitorTopology, PowerState,
        VideoInterfaceInfo,
    },
    MonitorDetails,
};
...
fn generate_populated_details(count: usize) -> Vec<MonitorDetails> {
    let mock_edid = EdidData {
        ...
        hdr_caps: HdrMetadata {
            ...
            supports_hlg: true,
            max_luminance_cd_m2: Some(400.0),
            max_frame_average_luminance_cd_m2: Some(350.0),
            min_luminance_cd_m2: Some(0.001),
        },
        audio_caps: AudioCapabilities {
            extra_audio_descriptors_count: 0,
    ...
    (0..count)
        .map(|i| MonitorDetails {
            ...
            device_path:
                "\\\\?\\DISPLAY#DEL42C2#7&21a11b&0&UID256#{e6f07b5f-ee97-4a90-b076-33f57bf4eaa7}"
                    .to_string(),
